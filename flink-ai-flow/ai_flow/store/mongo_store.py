...
class MongoStore(AbstractStore):
    ...
    def update_example(self, example_name: Text, support_type: ExampleSupportType, data_type: Text = None,
        ...
        try:
            example: DocumentExample = DocumentExample.objects.get(name=example_name)
            if example is None:
                return None
    ...
    def delete_example_by_name(self, example_name) -> Status:
        ...
        """
        try:
            example = DocumentExample.objects.get(name=example_name, is_deleted__ne=TRUE)
            if example is None:
                return Status.ERROR
            deleted_example_counts = DocumentExample.objects(name__startwith=deleted_character + example_name + deleted_character, is_deleted=TRUE).count()
    ...
    def update_project(self, project_name: Text, uri: Text = None, properties: Properties = None,
        ...
        try:
            project: DocumentProject = DocumentProject.objects.get(name=project_name, is_deleted__ne=TRUE)
            if project is None:
                return None
            if uri is not None:
                project.uri = uri
    ...
    def delete_project_by_name(self, project_name) -> Status:
        ...
        """
        try:
            project = DocumentProject.objects.get(name=project_name, is_deleted__ne=TRUE)
            if project is None:
                return Status.ERROR
            deleted_project_counts = DocumentProject.objects(
                name__startwith=deleted_character + project_name + deleted_character,
            ...
            for per_workflow_execution in project.workflow_execution:
                deleted_workflow_execution_counts = DocumentWorkflowExecution.objects(
                    name__startwith=deleted_character + per_workflow_execution.name + deleted_character,
                    is_deleted=TRUE).count()
                ...
                for per_job in per_workflow_execution.job_info:
                    deleted_job_counts = DocumentJob.objects(
                        name__startwith=deleted_character + per_job.name + deleted_character,
                        is_deleted=TRUE).count()
            ...
            for per_model in project.model_relation:
                    deleted_model_relation_counts = DocumentModelRelation.objects(
                        name__startwith=deleted_character + per_model.name + deleted_character,
                        is_deleted=TRUE).count()
                    ...
                    for model_version in per_model.model_version_relation:
                        deleted_model_version_relation_counts = DocumentModelVersionRelation.objects(
                            name__startwith=deleted_character + model_version.version + deleted_character,
                            is_deleted=TRUE).count()
            ...
            self._save_to_db([project] + project.workflow_execution + project.model_relation + job_list)
            return Status.OK
        except mongoengine.OperationError as e:
            raise AIFlowException(str(e))
    ...
    def delete_model_relation_by_name(self, model_name) -> Status:
        ...
        """
        try:
            model = DocumentModelRelation.objects.get(name=model_name, is_deleted__ne=TRUE)
            if model is None:
                return Status.ERROR
            deleted_model_counts = DocumentModelRelation.objects(
                name__startwith=deleted_character + model.name + deleted_character,
                is_deleted=TRUE).count()
            ...
            self._save_to_db([model] + model.model_version_relation)
            return Status.OK
        except mongoengine.OperationError as e:
            raise AIFlowException(str(e))
    ...
    def update_workflow_execution(self, execution_name: Text,
        ...
        try:
            workflow_execution: DocumentWorkflowExecution = DocumentWorkflowExecution.objects.get(name=execution_name, is_deleted__ne=TRUE)
    ...
    def update_workflow_execution_end_time(self, end_time, execution_name):
        ...
        """
        try:
            workflow_execution = DocumentWorkflowExecution.objects.get(name=execution_name)
            if workflow_execution is None:
                return UPDATE_FAIL
            workflow_execution.end_time = end_time
            workflow_execution.save()
    ...
    def delete_workflow_execution_by_name(self, execution_name) -> Status:
        ...
        """
        try:
            execution = DocumentModelRelation.objects.get(name=execution_name, is_deleted__ne=TRUE)
            if execution is None:
                return Status.ERROR
            deleted_execution_counts = DocumentWorkflowExecution.objects(
                name__startwith=deleted_character + execution.name + deleted_character,
                is_deleted=TRUE).count()
            ...
            for model_version in execution.model_version_relation:
                deleted_model_version_counts = DocumentModelVersionRelation.objects(
                    version__startwith=deleted_character + model_version.version + deleted_character,
                    is_deleted=TRUE).count()
                model_version.is_deleted = TRUE
                model_version.version = deleted_character + model_version.version + deleted_character + str(
                    deleted_model_version_counts + 1)
            self._save_to_db([execution] + execution.job_info + execution.model_version_relation)
            return Status.OK
        except mongoengine.OperationError as e:
            raise AIFlowException(str(e))
    ...
    def get_model_version_relation_by_version(self, version_name, model_id) -> Optional[ModelVersionRelationMeta]:
        ...
        """
        model_version_result = DocumentModelVersionRelation.objects(version=version_name, model_uuid=model_id, is_deleted__ne=TRUE)
    ...
    def list_model_version_relation(self, model_id, page_size, offset) -> Optional[List[ModelVersionRelationMeta]]:
        ...
        """
        model_version_result = DocumentModelVersionRelation.objects(model_uuid=model_id, is_deleted__ne=TRUE).skip(offset).limit(page_size)
        if len(model_version_result) == 0:
            return None
        model_version_list = []
    ...
    def delete_model_version_relation_by_version(self, version, model_id) -> Status:
        ...
        """
        try:
            model_version = DocumentModelVersionRelation.objects.get(version=version, model_uuid=model_id, is_deleted__ne=TRUE)
            if model_version is None:
                return Status.ERROR
            deleted_model_version_counts = DocumentModelVersionRelation.objects(
                version__startwith=deleted_character + version + deleted_character,
                is_deleted=TRUE).count()
            model_version.is_deleted = TRUE
            model_version.version = deleted_character + model_version.version + deleted_character + str(
                deleted_model_version_counts + 1)
    ...
    def update_job(self, job_name: Text, job_state: State = None, properties: Properties = None,
        ...
        try:
            job: DocumentJob = DocumentJob.objects.get(name=job_name, is_deleted__ne=TRUE)
            if job is None:
                return None
    ...
    def update_job_state(self, job_state: State, job_name):
        ...
        """
        try:
            job_update = DocumentJob.objects.get(name=job_name)
    ...
    def delete_job_by_name(self, job_name) -> Status:
        ...
        """
        try:
            job = DocumentJob.objects.get(name=job_name, is_deleted__ne=TRUE)
            if job is None:
                return Status.ERROR
            deleted_job_counts = DocumentJob.objects(
                name__startwith=deleted_character + job_name + deleted_character,
                is_deleted=TRUE).count()
            job.is_deleted = TRUE
    ...
    def update_artifact(self, artifact_name: Text, data_format: Text = None, description: Text = None,
        ...
        try:
            artifact: DocumentArtifact = DocumentArtifact.objects.get(name=artifact_name, is_deleted__ne=TRUE)
            if artifact is None:
                return None
            if data_format is not None:
    ...
    def delete_artifact_by_name(self, artifact_name) -> Status:
        ...
        """
        try:
            artifact = DocumentJob.objects.get(name=artifact_name, is_deleted__ne=TRUE)
            if artifact is None:
                return Status.ERROR
            deleted_artifact_counts = DocumentArtifact.objects(
                name__startwith=deleted_character + artifact_name + deleted_character,
                is_deleted=TRUE).count()
    ...
    def update_registered_model(self, registered_model, model_name=None, model_type=None, model_desc=None):
        ...
        if registered_model is None:
            ...
            try:
                ...
                self._save_to_db([registered_model] + registered_model.model_version)
                return registered_model.to_meta_entity()
            except mongoengine.OperationError as e:
                raise AIFlowException(
    ...
    def _count_deleted_model_version(cls, model_version):
        ...
        return DocumentModelVersion.objects(model_name=model_name,
                                            model_version__startwith=deleted_character + model_version + deleted_character,
    ...
    def delete_metric_meta(self, uuid: int):
        try:
            metric_meta_table = DocumentMetricMeta.objects.get(uuid=uuid)
    ...
    def delete_metric_summary(self, uuid: int):
        try:
            metric_summary_table = DocumentMetricSummary.objects.get(uuid=uuid)
    ...
    def update_metric_meta(self,
        ...
        """
        try:
            metric_meta_table: DocumentMetricMeta = DocumentMetricMeta.objects.get(uuid=uuid, is_deleted__ne=TRUE)
            if name is not None:
    ...
    def update_metric_summary(self,
        ...
        """
        try:
            metric_summary_table = DocumentMetricMeta.objects.get(uuid=uuid, is_deleted__ne=TRUE)
    ...
    def get_metric_meta(self, name) -> Union[None, MetricMeta]:
        ...
        """
        try:
            metric_meta_table = DocumentMetricMeta.objects.get(name=name, is_deleted__ne=TRUE)
    ...
    def get_dataset_metric_meta(self, dataset_id) -> Union[None, MetricMeta, List[MetricMeta]]:
        ...
        """
        try:
            metric_meta_tables = DocumentMetricMeta.objects(dataset_uuid=dataset_id,
    ...
    def get_metric_summary(self, metric_id) -> Optional[List[MetricSummary]]:
        ...
        """
        try:
            metric_summary_tables = DocumentMetricMeta.objects(metric_id=metric_id,
                                                               is_deleted__ne=TRUE)
    ...
    def update_member(self, server_uri, server_uuid):
        try:
            member = DocumentMember.objects.get(server_uri=server_uri)
            if member is None:
    ...
    def clear_dead_members(self, ttl_ms):
        try:
            member = DocumentMember.objects.get(update_time__lt=time.time_ns() / 1000000 - ttl_ms)
            member.delete()
        except Exception as e:
